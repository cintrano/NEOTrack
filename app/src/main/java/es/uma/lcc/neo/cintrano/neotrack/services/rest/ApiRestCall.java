...
public class ApiRestCall extends AsyncTask<Float, Void, String> {
    ...
    protected String doInBackground(Float... params) {
        ...
        if (params[1] != 0.0f && params[2] != 0.0f) {
            spot.setLatitude(params[1]);
            spot.setLongitude(params[2]);

            SharedPreferences.Editor editor = pref.edit();
            editor.putFloat(PREFERENCE_LATITUDE, spot.getLatitude());
            editor.putFloat(PREFERENCE_LONGITUDE, spot.getLongitude());
            editor.apply();
        } else {
            spot.setLatitude(pref.getFloat(PREFERENCE_LATITUDE, 0.0f));
            spot.setLongitude(pref.getFloat(PREFERENCE_LONGITUDE, 0.0f));
        }
